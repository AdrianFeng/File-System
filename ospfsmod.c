 ...
 *
 * EXERCISE: Finish 'ospfs_dir_readdir' and 'ospfs_symlink'. EXERCISE FINISHED
 */
...
//
//   EXERCISE: Finish implementing this function. EXERCISE FINISHED
...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	uint32_t Dir_Size = dir_oi->oi_size; //total directory size
	uint32_t f_pos = filp->f_pos;
	int r = 0;		/* Error return value, if any */
	int ok_so_far = 0;	/* Return value from 'filldir' */
	uint32_t Current_Location; // record the current location and its unit is byte
	uint32_t Current_Type; // corresponding file type
	char* Name;
	int Length;
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		Current_Location=  (f_pos -2)* 128; // minus 2 means that deduct the . and .. default directory
		                                    //128 is size in byte for each directory content entry
		/* If at the end of the directory, set 'r' to 1 and exit
		 * the loop.  For now we do this all the time.
		 *
		 * EXERCISE: Your code here */
		/*************************************************************/
		/*check to see if the current file position exceed the file size which is also the current
		 directory size*/
		//check ot see if the current location exceed the file size
		if (Current_Location >=Dir_Size ){
		  // ince the size exceed so change the return value as 1 and break from the loop
		r = 1;	       
		break;		
		}
		/*************************************************************/
		 ...
		 */
		//od is the next sub-directory data structure and call the function to get it 
		od =  ospfs_inode_data(dir_oi, Current_Location);
		// check to see if its contained inode data is 0 and if it is 0 go to next one 
		if( od->od_ino == 0 )
		  {
		    f_pos+=1; // go to next location 
		    continue;
		  }
		//entry_oi is next entry data and call the function to get it 
		entry_oi = ospfs_inode(od->od_ino);
		// get the current file type 
		Current_Type= entry_oi->oi_ftype;
		Name = od->od_name;
		Length =strlen(od->od_name);
		// check to see the current file type and pass it in the ok_so_far function 
		if ( Current_Type == OSPFS_FTYPE_REG ){
		  ok_so_far = filldir(dirent,Name,Length,f_pos, od->od_ino,DT_REG);
		}
		else if (Current_Type == OSPFS_FTYPE_SYMLINK ){
		  ok_so_far = filldir(dirent,Name,Length,f_pos, od->od_ino,DT_LNK);
		}
		else if (Current_Type == OSPFS_FTYPE_DIR){
		  ok_so_far = filldir(dirent,Name,Length,f_pos, od->od_ino,DT_DIR);
		}
		else {
		  // if goes into this loop and it means it is an unrecoginzed file type and we should return 
		  // error number -1 and break the loop;
		  r=-1;
		  eprintk("unable to recoginze the file type! \n");
		  break;
		}
		if (ok_so_far >= 0)
		  f_pos+=1;
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
	...
	ospfs_inode_t* node =ospfs_inode(od->od_ino);
	od->od_ino = 0;
	node->oi_nlink--;
	if(node->oi_ftype==OSPFS_FTYPE_REG)
	  {
	    if(node->oi_nlink==0) //means there is nothing point to this file
	      {
		int rev=change_size(node,0);
		if(rev==0)
		  {
		    printk("remove \n");
		  }
	...
	else if(node->oi_ftype==OSPFS_FTYPE_SYMLINK)
	  {
	    node->oi_nlink=0;
	    node->oi_size=0;
	  }
...
allocate_block(void)
{
  /* EXERCISE: Your code here */ //finished;
  void *bitmap=ospfs_block(OSPFS_FREEMAP_BLK);
  uint32_t i;
  for(i=0;i<ospfs_super->os_nblocks;i++)
    {
      if(bitvector_test(bitmap,i)) //means this block is free 
	{
	  //when it is free . set to no free which is set to 0
	  bitvector_clear(bitmap,i);
	  return i; //return index i;
	}
    }
  return 0; //means the disk is full
}
...
free_block(uint32_t blockno)
{
  ...
  uint32_t blocksizeofinode=OSPFS_INODESIZE*ospfs_super->os_ninodes/OSPFS_BLKSIZE;
   uint32_t Y;
   void *bitmap;
  if((OSPFS_INODESIZE*ospfs_super->os_ninodes)%OSPFS_BLKSIZE!=0) //if there is not remainder dont need to add 1 otherwise add 1;
    {
      blocksizeofinode++; //at least one block;
    }
 
  Y=ospfs_super->os_firstinob+blocksizeofinode;//find the y which is x+ blocksize of inodeblocks;
  if(blockno<Y)
    {
      printk("this blockno is lessthen boot sector superblock ,free block...  %d ",blockno);
    }
  bitmap=ospfs_block(OSPFS_FREEMAP_BLK);
  if(!(bitvector_test(bitmap,blockno))) //means this block is not free 
	{
	  //when it is not free . set to no free which is set to 1
...
// According to the question, we know that the maximum number of blocks can a file have is 
// (OSPFS_NDIRECT	 + OSPFS_NINDIRECT	    + OSPFS_NINDIRECT * OSPFS_NINDIRECT)
// That means only one double indirect allocated blocks can be used and only one more 
// sigle indirect allocated blocks can be used as well. And the rest are direct blocks 
// so when b exceed the  (OSPFS_NDIRECT + OSPFS_NINDIRECT), we should use double indirect
//allocated locks.  
...
indir2_index(uint32_t b)
{
	if (b >=OSPFS_MAXFILEBLKS){
	  printk(" number b exceed the maximum value, please check");
	}
...
indir_index(uint32_t b)
{
	...
	if ( indir2_index(b) == 0 ){
	  temp = b+1 - (OSPFS_NDIRECT + OSPFS_NINDIRECT + 1);
...
direct_index(uint32_t b)
{
  uint32_t temp;
  if (b >= OSPFS_MAXFILEBLKS){
    eprintk(" number b exceed the maximum value, please check");
  }
  if (b < OSPFS_NDIRECT )
    {
      return b;
...
add_block(ospfs_inode_t *oi)
{
	...
	int32_t Double = indir2_index(n);
	int32_t Single = indir_index(n);
	int32_t Direct = direct_index(n);
	int32_t i =0;
	uint32_t numberofint= OSPFS_BLKSIZE/4;
	uint32_t *block = NULL;
	uint32_t *block2 = NULL;
	uint32_t *block3 = NULL;
	uint32_t *block4 =NULL ;
	...
	if (Single == -1)
	{
	  ...
	  /*if ( oi->oi_direct[Direct] != 0)
		...
		{
			oi->oi_direct[Direct]= allocated[0];
			block = (uint32_t*) ospfs_block(oi->oi_direct[Direct]);
	...
	if(Single == 0)
	{
	  //printk("nihao2 \n");
		if (oi->oi_indirect == 0)
		{
			...
			block = (uint32_t*) ospfs_block(allocated[0]);
			for (i=0; i< numberofint; i++)
			{
			  block[i] = 0;
			}
		...
		block2=(uint32_t*) ospfs_block(oi->oi_indirect);
		...
		block2[Direct]=allocated[1];
		block3=(uint32_t*) ospfs_block(allocated[1]);
		for (i=0; i< numberofint; i++)
		{
		  block3[i] = 0;
		}
	...
	if (Double == 0)
	{
	  //printk("nihao 3 \n");
		...
		block2 = (uint32_t*) ospfs_block(oi->oi_indirect2);
		if (block2[Single] ==0)
		{
			allocated[1]=allocate_block();
			if (allocated[1] == 0 )
			...
			block2[Single] = allocated[1];
		}
		else
		{
			block3 = (uint32_t*) ospfs_block(block2[Single]);
		}
		 /* if (block3[Direct] !=0) 
	    {
	      if (allocated[1] != 0){
		...
		oi->oi_indirect2 =0;
	    	...
	    	if (block3[Direct] ==0)
		  {
	    	...
	    	block4=(uint32_t*) ospfs_block(block3[Direct]);
		for (i=0; i< numberofint; i++)
		  {
		    block4[i] = 0;
		  }
...
remove_block(ospfs_inode_t *oi)
{
	// current number of blocks in file
  	uint32_t n = ospfs_size2nblocks(oi->oi_size);
        int32_t Double = indir2_index(n-1);
	int32_t Single = indir_index(n-1);
	int32_t Direct = direct_index(n-1);
	uint32_t *block;
	uint32_t *block2;
	if (n == 0 )
	{
		return -EIO; // this means that there is no block inside this file right now,
					 // remove will cause error. 
	}
	if ( Single == -1)
	{
		if (oi->oi_direct[Direct] == 0)
	...
	if (  Single == 0)
	{
		...
		block = (uint32_t*) ospfs_block(oi->oi_indirect); 
		if (block[Direct] ==0)
		{
			return -EIO;
		}
		free_block(block[Direct]);
		block[Direct]=0;
		if (Direct == 0)
		{
	...
	if (Double ==0)
	{
		...
		block = (uint32_t*) ospfs_block(oi->oi_indirect2); 
		if (block[Single] == 0)
		{
			return -EIO;
		}
		block2 =(uint32_t*) ospfs_block(block[Single]);
		 if (block2[Direct] == 0)
		{
			return -EIO;
		}
		free_block(block2[Direct]);
		block2[Direct] = 0;
		if (Direct == 0)
		{
			free_block(block[Single]);
			block[Single] =0;
		}
		if (Single == 0)
		{
...
//
//   EXERCISE: Finish off this function. Finished
static int
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	uint32_t old_size = oi->oi_size;
	int r = 0;
	int32_t numberofcount =0;
	...
	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
	   ...
	   if (r == -ENOSPC)
	   {
	   		for (;numberofcount>1;numberofcount--)
	   		{
	   			r=remove_block(oi);
	   			 if ( r == -EIO)
	   			 {
...
//   when you're done.
//
//   EXERCISE: Complete this function.Finished
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if ( (Position + count) > oi->oi_size)
	{
		count = oi->oi_size - Position;
	}
	...
	while (amount < count && retval >= 0) {
		...
		Current_Location = Position% OSPFS_BLKSIZE;
		data = data + Current_Location;
		Absolute = count -amount;
		if (( OSPFS_BLKSIZE - Current_Location)>= Absolute )
		{
			if(copy_to_user(buffer, data, Absolute))
			{
				retval=-EFAULT;
				goto done;
			}
		...
		{
			if(copy_to_user(buffer, data, (OSPFS_BLKSIZE - Current_Location)))
			{
				retval=-EFAULT;
				goto done;
			}
			n =OSPFS_BLKSIZE - Current_Location;
		}
		// Figure out how much data is left in this block to read.
		// Copy data into user space. Return -EFAULT if unable to write
		// into user space.
		// Use variable 'n' to track number of bytes moved.
		/* EXERCISE: Your code here */ //EXERCISE FINISHED 
		buffer += n;
		amount += n;
		*f_pos += n;
	}
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if(filp->f_flags & O_APPEND) // add this to support the append mode
	  {
	  *f_pos = oi->oi_size;
	   Position = *f_pos;
	  }
	if (( Position + count) >= oi->oi_size )
	{
		retval = change_size(oi, Position + count);
		if (retval <0)
		{
			goto done;
		}
	...
	while (amount < count && retval >= 0) {
		...
		if (( OSPFS_BLKSIZE - Current_Location)>= Absolute)
		{
			if(copy_from_user(data, buffer, Absolute))
			{
				retval=-EFAULT;
				goto done;
			}
		...
		{
			if(copy_from_user(data,buffer, (OSPFS_BLKSIZE - Current_Location)))
			{
				retval=-EFAULT;
				goto done;
			}
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
  ...
  for(offset=0;offset<dir_oi->oi_size;offset+=OSPFS_DIRENTRY_SIZE)
    {
      ospfs_direntry_t * mydirentry=ospfs_inode_data(dir_oi,offset);
      if(mydirentry->od_ino==0) //find the empty directory;
	return ospfs_inode_data(dir_oi,offset);
    }
  //if not find the empty directory need to create one by add block
  if(add_block(dir_oi)<0)
    return ERR_PTR(add_block(dir_oi));
  //success 
  return ospfs_inode_data(dir_oi,offset);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
  ...
  ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino);
  uint32_t offset;
  ospfs_direntry_t * mydirentry;
  ospfs_direntry_t* newfile;
  ospfs_inode_t* ha;
  ...
  for(offset=0;offset<dir_oi->oi_size;offset+=OSPFS_DIRENTRY_SIZE)
    {
      mydirentry=ospfs_inode_data(dir_oi,offset);
      //find the empty directory;
      if(!strcmp(mydirentry->od_name,dst_dentry->d_name.name)&&mydirentry->od_ino!=0)
	return -EEXIST;
    }
  newfile=create_blank_direntry(dir_oi);
  ha=ospfs_inode(src_dentry->d_inode->i_ino);
  if(IS_ERR(newfile))
    {
      return -ENOSPC;
    }
    ...
    {
      ha->oi_nlink++;
      newfile->od_ino=src_dentry->d_inode->i_ino;
      memcpy(newfile->od_name,dst_dentry->d_name.name,dst_dentry->d_name.len);
      newfile->od_name[dst_dentry->d_name.len]='\0';
    }	
  return 0;
  }
// ospfs_create
...
//
//   EXERCISE: Complete this function. finish;
	...
	for(offset=0;offset<dir_oi->oi_size;offset+=OSPFS_DIRENTRY_SIZE)
	  {
	   mydirentry=ospfs_inode_data(dir_oi,offset);
	   //find the empty directory;
	    if(!strcmp(mydirentry->od_name,dentry->d_name.name)&&mydirentry->od_ino!=0)
	      return -EEXIST;
	  }
	newfile=create_blank_direntry(dir_oi);
	if(IS_ERR(newfile))
	...
	else
	  {
	    ...
	    if(entry_ino!=ospfs_super->os_ninodes)
	      {
		newfile->od_ino=entry_ino;
		memcpy(newfile->od_name,dentry->d_name.name,dentry->d_name.len);
		newfile->od_name[dentry->d_name.len]='\0';
		printk("new file name is %s ",newfile->od_name);
		ha->oi_nlink=1;
		ha->oi_ftype=OSPFS_FTYPE_REG;
		ha->oi_mode=mode;
		ha->oi_size=0;
	      }
	    else
  ...
  printk("symlink \n");
	...
	if(IS_ERR(newfile))
	...
	else
	  {
		...
		newfile->od_ino=entry_ino;
		memcpy(newfile->od_name,dentry->d_name.name,dentry->d_name.len);
		ospfs_symlink_inode_t *node=(ospfs_symlink_inode_t *)ospfs_inode(entry_ino);
		node->oi_size=strlen(symname);
		node->oi_ftype=OSPFS_FTYPE_SYMLINK;
		node->oi_nlink=1;
	  	memcpy(&node->oi_symlink,symname,strlen(symname));
	      }
	    else
	      {
		return -ENOSPC;
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	if(strncmp(oi->oi_symlink,"root?",5)==0)
	  {
	    //printk("hahah\n");
	    for(index=5;index<oi->oi_size;index++)
	      {
		if(oi->oi_symlink[index]==':')
		  {
		    // printk("the index is %d ",index);
		    break;
		  }
	      }
	    if(current->uid==0) //first path
	      {
		oi->oi_symlink[index]='\0';
		//printk("name is %s",oi->oi_symlink+5);
		nd_set_link(nd,oi->oi_symlink+5);
	      }
	    else //second path
	      {
		//printk("name is %s ",oi->oi_symlink+index+1);
		nd_set_link(nd,oi->oi_symlink+index+1);
	      }
	  }
	else
	  {
